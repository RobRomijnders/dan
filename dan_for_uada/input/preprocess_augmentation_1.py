...
def preprocess_train(images, labels, params=None):
  ...
  images, labels = random_color(images, labels, params)
  ...
  return tf.squeeze(proimages, 0), tf.squeeze(prolabels, 0)
...
def preprocess_predict(image, params):
  # image: 3D Tensor, tf.uint8
  mean = .5
...
def preprocess_evaluate(images, labels=None, params=None):
  # all are TF Tensors
  mean=.5
  proimages = (images - mean)/mean
  prolabels = labels
